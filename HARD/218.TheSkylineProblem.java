...
// 
// Related Topics 树状数组 线段树 数组 分治 有序集合 扫描线 堆（优先队列） 👍 657 👎 0
...
public class TheSkylineProblem {
    ...
    class Solution {
        public List<List<Integer>> getSkyline(int[][] buildings) {
            ...
            for (int bond : list) {
                while (idx<n&&buildings[idx][0]<=bond){
                    queue.offer(new int[]{buildings[idx][1],buildings[idx][2]});
                    idx++;
                }
                while (!queue.isEmpty()&&queue.peek()[0]<=bond){
                    queue.poll();
                }
                int maxn=queue.isEmpty()?0:queue.peek()[1];
                if(res.isEmpty()||maxn!=res.get(res.size()-1).get(1)){
                    res.add(Arrays.asList(bond,maxn));
                }
            }
            return res;
        }
